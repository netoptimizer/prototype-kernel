...
static int benchmark_slab_bulk_new_API(
	struct time_bench_record *rec, void *data)
{
#define MAX_BULK 250
	void *objs[MAX_BULK];
	...
	slab = kmem_cache_create("slab_bench_test5", sizeof(struct my_elem),
				 0, SLAB_HWCACHE_ALIGN, NULL);
	time_bench_start(rec);
	/** Loop to measure **/
	for (i = 0; i < rec->loops; i++) {
...
void bulk_test(int bulk)
{
	...
	time_bench_loop(loops/bulk, bulk, "kfree_bulk", NULL,
			benchmark_slab_bulk_new_API);
	cond_resched();
}
...
static int __init slab_bulk_test05_module_init(void)
{
	if (verbose)
		pr_info("Loaded\n");
...
module_init(slab_bulk_test05_module_init);

static void __exit slab_bulk_test05_module_exit(void)
{
	if (verbose)
		pr_info("Unloaded\n");
}
module_exit(slab_bulk_test05_module_exit);
